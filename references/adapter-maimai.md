...
## 六、返回内容的形态实测（能力层：这个通道能拿到什么质量的数据）

> ⚠️ 下面的关键词与命中内容是**实测证据**，用来说明"这个通道返回什么形态的数据"；
> **"这些内容对某个方向有没有价值"属内容层判断，不在本卡维护**（EVOLUTION §2.3）。

**返回内容里出现的字段级形态**：帖子正文常含**薪资结构/包体**（倍数×月数、作息、业务线），且带发帖人身份标签——这是本机已接通道中**只有这一条能拿到**的数据形态：
