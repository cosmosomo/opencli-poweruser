...
牛客 `search` 对不同词性的表现差异极大：

| 词类型 | 效果 | 实测 |
|---|---|---|
| **公司名 / 岗位名** | ✅ **高质量真面经** | 搜某细分行业词 → 命中该行业公司的一面真题、某外企实习体验连帖 |
| **技术概念词** | ❌ **被 SEO 垃圾污染** | "光谱" → 5 条里 4 条是"路亿市场策略"之类的市场研究报告标题党 |

→ 初版规则（2026-09-16）：在牛客搜公司名和岗位名，不要搜技术概念词。

#### ⚠️ 2026-09-17 补测：上面那条规则只对了一半

另一批实测（35+ 篇真面经）里，**技术概念词照样 🔥 极高命中**：

| 词 | 结果 |
|---|---|
| `大模型 算法 面经` | GRPO vs DPO、OPD 自蒸馏、RAG 边界、bge-m3、DeepSeek 架构、多模态幻觉治理 |
| `笔试 难度 算法` | 2026 届笔试新形态：算法手撕 + 八股选择 + **AI Coding（Prompt 代码生成）** |
| `秋招 时间线` / `提前批 时间线` | 各厂投递→笔试→一面→二面→HR 面→意向书的真实日历 |

和 09-16 的"光谱 → 4/5 是市场报告"并不矛盾——差别在**有没有场景词**：

| 查询形态 | 结果 | 为什么 |
|---|---|---|
| 概念词**单搜**（`光谱`） | ❌ SEO 垃圾 | 概念词是市场研究报告标题党的主战场，牛客站内 SEO 页会盖过社区帖 |
| 概念词 **+ 场景词**（`面经` / `难度` / `时间线` / `offer`） | ✅ 高命中 | 场景词是社区帖独有的自称词，SEO 页不会用 |

→ **合并后的规则**：
> 牛客 `search` 搜**公司名 / 岗位名**，或**概念词 + 场景词**（面经 / 难度 / 时间线 / offer选择 / 投票）。
> **不要单搜裸概念词**——那是 SEO 垃圾区。
...
实测 `search "spectroscopy algorithm" --limit 3`：能返回，但 **`title` / `salary` / `tags` 三个字段全空**，
