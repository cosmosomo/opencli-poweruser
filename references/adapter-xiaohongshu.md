...
## search 节奏再验证（2026-09-17 AntiGravity 委托探针批次）

- **35s 间隔 + 5 词/批 = 零限流**：补搜 5 个关键词（博士简历/央国企简历/算法岗简历/选调生简历/理工科简历），每个间隔 35 秒，全部正常返回，零拦截。
- **此前 09-16 批次**：30s 间隔 + 6 词/批，在第 6-7 词触发限流（四连空数组）。
- **结论**：保守取 **35s 间隔 + ≤5 词/批** 是可持续节奏。比 30s+6 词只多 25% 时间，但避免了限流冷却（≥60s），实际更快。
- **note 通道不受影响**：search 限流期间 note/comments 仍可正常调用，可转精读。
...
## 错误码 300017「安全限制 - 访问链接异常」（2026-09-21 新增）

> 来源：用户频繁遇到浏览器打开笔记 URL 时跳转到黑色安全限制页面。
...
这是小红书**网页级风控拦截页**，与 search 返回空 `[]` 是不同机制：

| 机制 | 表现 | 触发层 | 恢复方式 |
|---|---|---|---|
| search 空 `[]` | API 返回空数组，无报错 | API 层限流 | 冷却 ≥60s |
| **300017** | 浏览器跳黑色拦截页 | **页面级风控** | **冷却 + 重建会话** |

### 触发原因（按概率排序）

1. **xsec_token 过期或复用**：search 返回的 URL 里 `xsec_token` 有 TTL（约数小时），隔一段时间再打开就触发 300017
2. **冷启动直接打开笔记 URL**：浏览器没有先访问过首页/search，直接 deep-link 打开笔记页，行为特征像爬虫
3. **批量连续页面跳转**：短时间内连续 open 多个不同笔记 URL，页面级行为指纹异常
4. **IP/账号风控累积**：近期频繁使用后风控阈值降低，间歇性触发

### 应对策略

1. **拿到 URL 尽快用**：search 结果 → note 精读不要间隔太久（同一批次内完成，不要隔天）
2. **先暖会话再 deep-link**：批量操作前先 `browser open` 首页 `https://www.xiaohongshu.com`，等 2 秒，再开笔记页
3. **遇到 300017 不要反复重试**：立即停止，冷却 ≥60 秒，期间可以 `browser open` 首页正常浏览
4. **重建会话**：如果持续 300017，用 `--window foreground --site-session persistent` 重建一次会话
5. **note 命令不受影响**：OpenCLI 的 `xiaohongshu note` 命令走的是浏览器扩展注入，不是页面导航，通常不会触发 300017。300017 主要出现在 `browser open` 直接导航时

### 与 OpenCLI note 命令的关系

- `opencli xiaohongshu note "<url>"` → 通过扩展在已打开页面注入 JS，**不触发页面导航**，基本不会遇到 300017
- `opencli browser xiaohongshu open "<url>"` → 真实页面导航，**可能触发 300017**
- 结论：需要读笔记内容时，优先用 `note` 命令而不是 `browser open` 后再 eval

---

*本文件最后更新：2026-09-21（新增 300017 错误码文档）*
